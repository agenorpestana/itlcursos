...
echo -e "${GREEN}=== GERENCIADOR ITL CURSOS ===${NC}"
...
read -p "Digite o domínio do site (ex: cursos.meusite.com): " DOMAIN_NAME
read -p "Digite a porta para este sistema (ex: 3003): " APP_PORT
...
EOF
    ...
    read -p "Nome do Banco de Dados (ex: itl_cursos_db): " DB_NAME
    read -p "Nome do Usuário do Banco (ex: itl_user): " DB_USER
...
EOF
    pm2 delete itl-cursos-app 2>/dev/null || true
